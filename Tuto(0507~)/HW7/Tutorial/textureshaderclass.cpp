...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd, int menu)
{
	bool result;
	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, (WCHAR *)L"../Tutorial/texture.vs", (WCHAR *)L"../Tutorial/texture.ps", menu);
	if (!result)
	{
		return false;
	}
...
// This setup needs to match the VertexType stucture in the ModelClass and in the shader.
	...
	//}
	if(menu == 1) samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
	if(menu == 2) samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_POINT;
	else samplerDesc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR;
